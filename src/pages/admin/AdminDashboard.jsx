...
function TopperManager({ data, updateData }) {
  ...
  const [form, setForm] = useState({ name: '', year: '', percentage: '', imageUrl: '', program: '', rank: '', class: 'TY' });

  const f = v => setForm(p => ({ ...p, ...v }));
  
  const openAdd = () => { 
    setEditing(null); 
    // Smart defaulting for program based on class
    let defaultProg = '';
    if (filterClass === 'FY') defaultProg = 'F.Y. B.Sc. (Comp. Sci.)';
    else if (filterClass === 'SY') defaultProg = 'S.Y. B.Sc. (Comp. Sci.)';
    else if (filterClass === 'TY') defaultProg = 'T.Y. B.Sc. (Comp. Sci.)';
    else if (filterClass === 'M.Sc. I') defaultProg = 'M.Sc. (Comp. Sci.) – I';
    else if (filterClass === 'M.Sc. II') defaultProg = 'M.Sc. (Comp. Sci.) – II';
  ...
  const openEdit = item => {
    ...
    setForm({
      ...item,
      percentage: item.percentage || item.cgpa || item.marks || ''
    });
    setShowModal(true);
  };
  ...
  const handleSave = () => {
    ...
    delete finalForm.cgpa;
    delete finalForm.marks;

    if (editing) updated = data.toppers.map(x => x.id === editing.id ? { ...x, ...finalForm } : x);
    else updated = [...data.toppers, { ...finalForm, id: Math.max(0, ...data.toppers.map(x => x.id), 0) + 1 }];
    updateData('toppers', updated);
    setShowModal(false);
  };
  const handleDelete = id => updateData('toppers', data.toppers.filter(x => x.id !== id));

  const years = Array.from({ length: new Date().getFullYear() - 2015 + 1 }, (_, i) => (new Date().getFullYear() - i).toString());
  const classes = ['FY', 'SY', 'TY', 'M.Sc. I', 'M.Sc. II'];

  const filtered = data.toppers.filter(t => t.year?.toString() === filterYear && t.class === filterClass);
  const sorted = [...filtered].sort((a, b) => parseInt(a.rank) - parseInt(b.rank));
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6 bg-gray-50 p-4 rounded-2xl border border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <select 
            ...
            className="px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-red-100 transition-all"
          >
          ...
          <div className="flex bg-white p-1 rounded-lg border border-gray-200 gap-1">
            {classes.map(c => (
              <button
                ...
                className={`px-3 py-1 rounded-md text-[10px] font-bold transition-all ${filterClass === c ? 'bg-[#6b1a2a] text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
              >
                {c}
              </button>
            ))}
      ...
      <div className="space-y-2">
        {sorted.length === 0 ? <EmptyState msg={`No toppers found for ${filterClass} in ${filterYear}.`} /> : sorted.map(item => (
          <motion.div key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3 p-4 bg-white border border-gray-100 rounded-xl hover:shadow-md transition-all">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500">{item.class} • {item.program} • {item.year} • {item.percentage || item.cgpa || item.marks}</p>
            </div>
            <ActionBtns onEdit={() => openEdit(item)} onDelete={() => handleDelete(item.id)} />
          </motion.div>
        ))}
      ...
      {showModal && (
        <Modal title={editing ? 'Edit Topper' : 'Add Topper'} onClose={() => setShowModal(false)}>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Field label="Percentage"><Input value={form.percentage} onChange={v => f({ percentage: v })} placeholder="85.50%" /></Field>
            <Field label="Rank"><Input value={form.rank} onChange={v => f({ rank: v })} placeholder="1" /></Field>
            <Field label="Class">
              <select value={form.class} onChange={e => f({ class: e.target.value })} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-200 focus:bg-white transition-all text-gray-700">
