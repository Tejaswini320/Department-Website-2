...
export default function Administration() {
  ...
  useEffect(() => {
    if (location.state?.facultyId) {
      const faculty = data.faculty.find(f => f.id === location.state.facultyId);
      if (faculty) {
        setSelectedFaculty(faculty);
      }
    }
  }, [location.state, data.faculty]);
...
=======
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="mt-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            { name: 'Mr. Bharat Deshmukh', designation: 'Lab Assistant', imageUrl: '/Information/teacher img/Bharat Deshmukh.jpeg', qualification: "M.A., DOEACC 'O' Level", experience: '18 Years', contact: '9552818117', email: 'bharatdeshmukh1982@gmail.com' },
            { name: 'Mr. Deepak Athare', designation: 'Lab Attendant', imageUrl: '/Information/teacher img/Deepak Athare.jpeg', qualification: 'B.A.', experience: '15 Years', contact: '9823778134', email: 'deepakathare34@gmail.com' },
          ].map(staff => (
            <div
              key={staff.name}
              onClick={() => setSelectedFaculty(staff)}
              className="bg-white rounded-xl border border-gray-100 p-5 hover:shadow-md transition-all text-center cursor-pointer group hover:-translate-y-1 hover:border-primary/20"
            >
              <PlaceholderImage src={staff.imageUrl} aspect="passport" label="Staff Photo" />
              <h3 className="font-bold text-navy group-hover:text-primary transition-colors">{staff.name}</h3>
